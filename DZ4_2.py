...
# current_data = datetime.date

def current_valuta(text, current='USD'):
    current = current.upper()
    data_inf = text.find('<CharCode>')
    data_inf2 = text.find('</CharCode>')
    current_name = text[data_inf + 10: data_inf2]
    if current_name == current:
        ...
        print(f'Сегодня {current_data}: Валютная пара {current_name}: номинал {current_nomin}: в RUR = {Value} ')
    else:
        ...
        if text == '':
            ...
            current_valuta(text, current)


current_valuta(result_text,'AUD')
